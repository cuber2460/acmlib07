 ...
 */
void lyndon(
		char * s   /* Słowo zaczynające się na pozycji 0:
		            *   2) s powinno być postaci xx, gdzie x jest prawdziwym słowem */
		,int n     /* Długość słowa s (licząc ew. podwojenie) */
		,int & suf /* 3) pozycja minimalnego leksykograficznie sufiksu */
		,int & cyk /* 2) pozycja minimalnego leksykograficznie przesunięcia cyklicznego */
		,bool * b  /* Tablica cięcia b */
	)
{
	for(int i = 0; i < n; i++) b[i] = false; /* tylko 1) */
