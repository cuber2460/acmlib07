...
pii cornacchia(int p, int d) {//zwarca (a, b) takie, że p = a^2 + db^2, lub (-1, -1) jeśli takie nie istnieją
...
bool can(int p, int d) {
	for (int i = 0; d * i * i <= p; ++i) if (is_sq(p - i * i * d)) return true;
	return false;
}
void test(int p, int d) {
	pii ans = cornacchia(p, d);
	if (ans.first == -1)
		assert(!can(p, d));
	else
		assert(ans.first * ans.first + ans.second * ans.second * d == p);
}
int main() {
	for (int p = 3; p < 1e7; ++p) if (is_prime(p)) {
		debug << imie(p);
		for (int d = 2; d < 100; ++d)
			test(p, d);
	}
